...
import com.pos.salon.adapter.ListAdapters.QuatationListAdapter;
...
public class QuatationListFragment extends Fragment {

    private RecyclerView quatationRecyclerView;
    private TextView txt_reurn_list_date_range, txt_customRange;
    private QuatationListAdapter quatationListAdapter;
    private ImageView open_filters, close_filters;
    ...
    private String startDate = "", endDate = "",firstDate="",lastDate="";
    private final ArrayList<QuatationListModel> quatationSalesList = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.fragment_quatation_list, container, false);
    }
    @Override
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        quatationRecyclerView = view.findViewById(R.id.list_quatation_Recycler);
        open_filters = view.findViewById(R.id.open_filters);
        ...
        quatationRecyclerView.setLayoutManager(mLayoutManager);
        quatationListAdapter = new QuatationListAdapter(getContext(),quatationSalesList);
        quatationRecyclerView.setAdapter(quatationListAdapter);
    ...
    public void listeners() {
        ...
        quatationListAdapter.setOnItmeClicked(new QuatationListAdapter.OnClicked() {
            @Override
            public void setOnClickedItem(int position) {

                if(isSaleView){
                    QuatationListModel listPosModel = quatationSalesList.get(position);

                    SaleDetailFragment ldf = new SaleDetailFragment();
                    Bundle args = new Bundle();
                    args.putString("transactionId", listPosModel.quatationId);
                    args.putString("from", "fromQuatationListFragment");
...
//                transaction.commit();
//                getFragmentManager().beginTransaction().replace(R.id.lay_quatation, ldf).commit();
                    ...
                    transaction.replace(R.id.lay_quatation, ldf);
...
//                txt_reurn_list_date_range.setText("");
        ...
        quatationRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
...
//                Toast.makeText(getContext(),"UnSelected Date is : " +date.toString(),Toast.LENGTH_LONG).show();
    ...
    public void quatationList() {
        ...
        if (retrofit != null) {
            ...
            Call<ResponseBody> call = apiService.quatationSaleList("sells/quotations",startDate,endDate,quatationSalesList.size());
                ...
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    ...
                    try {
                        if (response.body() != null) {
                            ...
                            Log.e("Quatation Sales List", responseObject.toString());
...
//                                salesList();
                                ...
                                for (int i = 0; i < dataObj.length(); i++) {
                                    ...
                                    QuatationListModel quatationListModel = new QuatationListModel();
                                    quatationListModel.quatationId = String.valueOf(data.getInt("id"));
                                    quatationListModel.quatationDate = data.getString("transaction_date");
                                    quatationListModel.quatationInvoiceNo = data.getString("invoice_no");
                                    quatationListModel.quatationCustomerName = data.getString("name");
                                    quatationListModel.quatationBusinessLocation = data.getString("business_location");
                                    quatationListModel.quatation_direct_sale = data.getInt("is_direct_sale");

                                    quatationSalesList.add(quatationListModel);
//                                    }

                                }

                                if(quatationSalesList.size()==0){
                                    txt_no_resut.setVisibility(View.VISIBLE);
                                }else {
                                    txt_no_resut.setVisibility(View.GONE);
                                }

                                quatationListAdapter.notifyDataSetChanged();
                            ...
                            AppConstant.hideProgress();
                            progressBar.setVisibility(View.GONE);
                            AppConstant.sendEmailNotification(getContext(),"sells/quotations API","(QuatationListFragment Screen)","Web API Error : API Response Is Null");
                            Toast.makeText(getContext(), "Could Not Load Quatation Sale List. Please Try Again", Toast.LENGTH_LONG).show();
                        }
